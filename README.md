...
However, you must be mindful of the resources you dedicate to your K8S cluster. To execute this demo smoothly, you need to dedicate to the underlying infrastructure that runs your K8S cluster (either a VM or a Docker container) at least **4 CPUs**, **8 GB of memory**, and **25 GB of disk**. Anything less than this will cause the pods to continuously crash and be recreated, making your K8S cluster unstable. Once your K8S cluster runs, the RDI deployment is fully automated using scripts in the `rdi-deploy` folder.
